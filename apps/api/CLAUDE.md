...
- **Hyperdrive query caching is DISABLED and must stay that way** (`--caching-disabled`, see [wrangler.toml](wrangler.toml)). It defaults to ON (60s + 15s SWR) and never invalidates on write, *below* the app layer — so it ignored the authenticated-request bypass in [src/lib/edge-cache.ts](src/lib/edge-cache.ts) and served owners their pre-write row after a save. Caching belongs at the app layer, where it can be session-aware and purged. The setting lives in Cloudflare's control plane, not the repo, so no deploy carries it — re-verify with `wrangler hyperdrive get <id>` after any config recreate.
- **Full-text search lives outside Prisma**, in [scripts/migrations/2026-08-04_build_search_vector.sql](scripts/migrations/2026-08-04_build_search_vector.sql) — the `searchVector Unsupported("tsvector")` column, the `builds_search_vector_update()` trigger function (weights: name=A, itemName=B, description=C, config `'english'`), its trigger, and the GIN index. Prisma can neither generate the trigger nor index the type, so **that file is the source of truth and any rebuild must apply it**; `prisma db push` alone drops the trigger and search silently stops matching with no error. Re-verify the live DB still matches it with [scripts/dump-search-schema.sql](scripts/dump-search-schema.sql) (read-only, runs in PlanetScale's SQL console) after any schema change. Two things that dump found on 2026-08-04, worth not re-learning: the trigger's `'english'` config must match the `to_tsquery('english', …)` in `searchBuildIds` or stemming disagrees silently, and **the GIN index never actually existed in production** despite this file and that function's comment both claiming it did — every search was a sequential scan until the migration above.
