...
- **Every accepted `PATCH /builds/:slug` writes a `BuildRevision`** in the same statement as the update, carrying the editor and a change list from [@arsenyx/shared/warframe/build-diff](../../packages/shared/src/warframe/build-diff.ts) (`diffBuildData` over old vs new `buildData`, plus the column-level fields it can't see — name, visibility, guide). It stores a *description*, never a snapshot: a change list is a few hundred bytes, `buildData` is 5–80 KB. The describe/fold/prune logic lives in [src/routes/\_build-revisions.ts](src/routes/_build-revisions.ts); `builds.ts` keeps only the route. Bursts of saves from one editing session are folded at **read** time (`foldRevisions`), not on write — folding on write would need the pre-burst document, which isn't kept. This table is also the only edit attribution that exists; `Build` records its author and nothing else.
...
- **Rate limiting is two-layered.** Cookie-session mutations go through `rateLimitUser` ([middleware/rate-limit.ts](src/middleware/rate-limit.ts)), which is Postgres-backed and therefore only covers *authenticated* callers. Anonymous traffic is capped at the Cloudflare edge by the `[[unsafe.bindings]]` rate-limit bindings in [wrangler.toml](wrangler.toml) — `ANON_READ_LIMITER` (public reads), `ANON_SEARCH_LIMITER` (typeahead), `AUTH_LIMITER` (all of `/auth/*`, all methods). Edge limiters run before any DB work and hold across isolates; Better Auth's own limiter does not (in-memory storage = one counter per isolate), which is why `AUTH_LIMITER` exists alongside it. `app.use("/auth/*", …)` must stay registered **before** the `/auth/*` handler in [src/index.ts](src/index.ts) or Hono silently skips it.
- **`securityHeaders` ([middleware/security.ts](src/middleware/security.ts)) is registered first in [src/index.ts](src/index.ts)** — outermost, so it also covers CORS preflights, `/auth/*`, and `onError` responses. It fills in `nosniff` / `default-src 'none'; frame-ancestors 'none'` / `no-referrer` only when a route hasn't set them, so per-route overrides win. Cache API responses have an immutable header guard, which is why [lib/edge-cache.ts](src/lib/edge-cache.ts) re-wraps its hits (`new Response(hit.body, hit)`) — return a raw cache hit and the header write throws. Wiring is pinned by [src/index.test.ts](src/index.test.ts).
- **`/img` must never serve `image/svg+xml`.** An SVG loaded as a top-level document runs script, and this origin holds the session cookies — `originGuard` blocks same-origin *mutations*, but reads (`/auth/get-session`, `/me/builds/export`, `/admin/users`) would all be exfiltratable. The gate compares the media type with parameters stripped, and the response carries `default-src 'none'; sandbox` as a second layer. See [routes/img.test.ts](src/routes/img.test.ts).
- **Every mutation parses its body through `parseJsonBody`** ([lib/validate.ts](src/lib/validate.ts)), which streams and aborts past a byte cap. Bare `c.req.json()` buffers the whole body before any check runs — don't reach for it.
- **`?page` is clamped** to `MAX_PAGE` in [routes/_query.ts](src/routes/_query.ts). Pagination is OFFSET-based, so an unbounded page number is a free way for an anonymous caller to make Postgres walk millions of rows. Don't remove the clamp to "support deep paging" — switch to keyset pagination instead.
...
- **Hyperdrive query caching is DISABLED and must stay that way.** It lives in Cloudflare's control plane, not the repo, so no deploy carries it and a recreated config re-enables it by default. Rationale, the reapply command, and the re-verify step are in the comment block above `[[hyperdrive]]` in [wrangler.toml](wrangler.toml).
- **Full-text search lives outside Prisma** — the `searchVector` column, its trigger, and the GIN index are all in a hand-applied migration. Touching the schema, the search query, or rebuilding the DB → read [docs/search.md](../../docs/search.md) first.
