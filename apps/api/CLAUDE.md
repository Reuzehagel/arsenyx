...
- **Hyperdrive query caching must stay DISABLED** (`--caching-disabled`, see [wrangler.toml](wrangler.toml)). It defaults to ON (60s) and never invalidates on write, *below* the app layer — so it ignores the session-cookie bypass in [src/lib/edge-cache.ts](src/lib/edge-cache.ts) and serves an owner their pre-write row after a save. Caching belongs at the app layer, where it can be session-aware and purged. The setting lives in Cloudflare's control plane, not the repo — re-verify after any config recreate.
