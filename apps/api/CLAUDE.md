...
- **Full-text search lives outside Prisma:** the `searchVector Unsupported("tsvector")` column, its GIN index, and the populating trigger are **not** in the schema or migrations — they exist only in the live DB. Any DB rebuild must come from a `pg_dump` of the live schema, **never `prisma db push`** (which would drop the GIN index + trigger and silently break search). Run [scripts/dump-search-schema.sql](scripts/dump-search-schema.sql) (read-only, works in PlanetScale's SQL console) to get the exact column/index/trigger/function definitions, then commit them as a dated migration in [scripts/migrations/](scripts/migrations/). **These should be a checked-in migration and are not yet** — until they are, this note is the only thing standing between a routine `db push` and silently broken search.
