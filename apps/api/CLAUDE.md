...
- **Every accepted `PATCH /builds/:slug` writes a `BuildRevision`** in the same statement as the update, carrying the editor and a change list from [@arsenyx/shared/warframe/build-diff](../../packages/shared/src/warframe/build-diff.ts) (`diffBuildData` over old vs new `buildData`, plus the column-level fields it can't see — name, visibility, guide). It stores a *description*, never a snapshot: a change list is a few hundred bytes, `buildData` is 5–80 KB. Bursts of saves from one editing session are folded at **read** time in `GET /:slug/revisions` (`foldRevisions`), not on write — folding on write would need the pre-burst document, which isn't kept. This table is also the only edit attribution that exists; `Build` records its author and nothing else.
